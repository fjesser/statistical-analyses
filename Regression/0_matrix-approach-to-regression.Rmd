...
$$SS_{Total} = \mathbf{y_c'y_c} = \mathbf{(y - \bar{y})' (y - \bar{y})} = \sum_{i=1}^{n} (y_i - \bar{y})^2$$
...
$$SS_{Regression} = \mathbf{\hat{Y}_C' \hat{Y}_C} = \begin{bmatrix}\hat{y}_{c1} & \ldots & \hat{y}_{cn} \end{bmatrix} \begin{bmatrix}\hat{y}_{c1} \\ \vdots \\ \hat{y}_{cn}\end{bmatrix} = \sum_{i=1}^{n} y_{ci}^2 = \sum_{i=1}^{n} (\hat{y}_i-\bar{y})^2$$
