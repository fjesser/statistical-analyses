...
Analysis of Variance (ANOVA) is a family of analyses to compare the mean of several samples. Hence, the outcome variable has to be at least metric scale of measurement and the predictor variable(s) has/have nominal scale of measurement.

Total variability of the outcome score is partitioned into variability that can be explained with the predictors and variability that can not be explained by the predictors. That is the reason why it is called analysis of variance (analysis = partitioning).
...
The ANOVA tests the omnibus hypothesis that all means are equal. If the ANOVA yields a significant result, then this means that at least one pair of means differ (is this true?). In order to further investigate which means differ, multiple comparisons are conducted. There are a lot of different approaches, that is why this topic is described in more detail in another section (link to multiple comparison). In the following, only a brief summary of multiple comparison is provided so that the rest of this section can be understood. Within the example analyses only the most prominent versions of multiple comparisons are shown. If a different approach is necessary, please see (link to multiple comparisons). 

a priori (planned comparison) vs. a posteriori (post-hoc)
...
link to contrast analysis on branch mult_comp
...
library(ggpubr) # extension of significance lines/stars for ggplot2 
...
1.  Independent residuals 
    + residuals are independent from another 
    + this assumption is fulfilled via the research design 
    + is for example violated through repeated measures 
    + violation increases hugely probability of committing an $\alpha$ error
2.  Homoskedasticity 
    + Equal variance of residuals between conditions 
    + is tested with Levene's test for equal variances 
    + The $F$-test is relatively robust against violations of the assumption of homoskedasticity if samples have the same size 
        - if variance in smaller sample is larger than in a larger sample, the $F$-test becomes too liberal (higher risk of committing an $\alpha$ error) 
        - if variance in smaller sample is smaller than in a larger sample, the $F$-test becomes too conservative (higher risk of committing an $\beta$ error) 
    + If there is variance heterogeneity (heteroskedasticity), the Brown-Forsythe-test or the Welch-test can be used which correct the result. No one of both is superior to the other.
3.  Normal distribution of residuals
    -   Since a residuum is the deviation of a measurement from the mean of the condition, measurements have to distribute normally within conditions 
    + On the basis of the central limit theorem, the $F$-test is relatively robust against violations against the normality assumption under large sample sizes. So you can argue this assumption away. 
    + If the violation of the normality assumption is due to outliers, robust variants of ANOVA can be conducted.
...
More than 1 factor (explanatory variable)

Introduction of interaction effect

- only presented for balanced designs! See (link to Non-orthogonal ANOVA) for unbalanced designs. In short, usually Type III sum of squares are used in unbalanced designs (explain)


It is also possible to analyze a factorial ANOVA with contrasts and get the same result (link to contrasts)

```{r}

# ggpubr facilitates process but there is no option for controlling for 
# multiple comparison
# therefore geom_signif is better because manual adjustment is possible

data_indep1 |> 
    mutate(seconddv = weight +10 |>  rnorm()) |> 
    pivot_longer(cols = c(weight, seconddv), names_to = "variable",
                 values_to = "value") |> 
    ggplot(aes(x = variable, y = value, fill = group)) +
    stat_summary(fun = mean, geom = "bar", position = "dodge") +
    geom_signif(comparison = list(c("ctrl", "trt1")),
                test =  "t.test")
...
Factor A with $j = 1, \ldots, g$ levels
- Factor B with $k = 1, \ldots, h$ levels

Total variability is the variability of the outcome variable without considering any predictor which can be partitioned into the variability of the effect of factor $A$, the effect of factor $B$, and the interaction effect between factor $A$ and $B$.
$$\begin{array}{rccll}
SS_{Total} & = & \sum_{k=1}^h \sum_{j=1}^g \sum_{i=1}^{n_{cell}} (y_{ijk} - \bar{Y})^2 \\
& = & SS_{Model} &+ &SS_{Residual}\\
&=& \overbrace{SS_A + SS_B + SS_{A \times B}} &+& SS_{Residual}
\end{array}$$

$$df_{Total} = N - 1$$

Observed score minus cell mean
$$SS_{Residual} = \sum_{k=1}^h \sum_{j=1}^g \sum_{i=1}^{n_{cell}} (y_{ijk} - \bar{Y}_{jk})^2 $$

$$df_{Residual} = N - (h\cdot g)$$


Model sum of squares
$$SS_{Model} = n_{cell} \cdot \sum_{k=1}^h \sum_{j=1}^g (\bar{Y}_{jk} - \bar{Y})^2$$
$$df_{Model} = df_{A} + df_{B} + df_{A \times B}$$

Sum of squares for factor $A$
$$SS_{A} = n_{cell} \cdot h \sum_{j=1}^g (\bar{Y}_{j\bullet} - \bar{Y})^2$$

$$df_{A} = g-1$$

Sum of squares for factor $B$
$$SS_{B} = n_{cell} \cdot g \sum_{k=1}^h (\bar{Y}_{\bullet k} - \bar{Y})^2$$

$$df_{B} = h-1$$

Sum of squares for interaction between factor $A$ and $B$

If there were no interaction effect, the cell means would be only due to the effect of A and B: 
$$\bar{Y}_{jk} = \bar{Y}_{j\bullet} + \bar{Y}_{\bullet k} - \bar{Y}$$
which is the same as:
$$\bar{Y}_{jk} - \bar{Y}_{j\bullet} - \bar{Y}_{\bullet k} + \bar{Y} = 0$$
Therefore, if the previous expression is not 0, then there is an interaction effect. Deviations from zero need to be squared and added together to get the sum of squares for the interaction effect.
$$SS_{A\times B} = n_{cell} \cdot \sum_{k=1}^h \sum_{j=1}^g (\bar{Y}_{jk} - \bar{Y}_{j\bullet} - \bar{Y}_{\bullet k} + \bar{Y})^2$$
$$df_{A \times B} = (g-1)\cdot (h-1)$$
$df_{A \times B}$ is the same principle as the $df$ for the $\chi^2$ (row-1 times column-1) (link to chi square part)


Creation of Mean Squares...

Calculation of $F$ tests

#### Interactions

different types of interaction
when no interaction lines are parallel

3 types of interaction

1) ordinal
    - all main effects can be interpreted globally, meaning over the levels of the other factor(s)
2) disordinal
    - no main effects can be interpreted globally
3) hybrid
    - not all but a subset of factors can be interpreted gobally
    
    
add a bunch of line graphs

### Assumptions of Two-Way ANOVA ###

1. Dependent Variable is metric
2. Homogeneity of variances
3. Independent and normal distributed residuals


#### Two-Way ANOVA by hand

This is to show the calculations 

minimal data example
again only for balanced designs; see non-orthogonal Factorial ANOVA (link to non-orthogonal)


Include sanity check: SS_T = SS_A ....



#### Data Two-Way ANOVA
```{r}
data_indep2 <- datasets::ToothGrowth |> 
    mutate(dose = factor(dose))

skim(data_indep2)
```
Note, the variable `dose` is an ordinal variable but is treated here as a nominal variable (link to scales of measurement).
...
Factor A with $j = 1, \ldots, g$ levels (j because it comes after i in the alphabet and i is used as the default iteration variable but it already taken for the number of cases; g for groups)
Factor B with $k = 1, \ldots, h$ levels (k because it comes after j in alphabet; h because it comes after g in alphabet)

Total sum of squares are outcome values minus the grad mean (overall mean, mean without considering any grouping). Represents total variability in $Y$
$$SS_{Total} = \sum_{k=1}^h \sum_{j=1}^g \sum_{i=1}^{n_{cell}} (y_{kji} - \bar{Y})^2$$
$$df_{Total} = N -1$$
also 
$$df_{Total} = df_{A} + df_{B} + df_{A\times B} + df_{Residual}$$



Residual sum of squares (also error sum of squares),
represents the average deviation of cases' values from their respective group mean.
$$SS_{Residual} = \sum_{k=1}^h \sum_{j=1}^g \sum_{i=1}^{n_{cell}} (y_{kji} - \bar{Y}_{kj})^2$$

$$df_{Residual} = n_{cell}-(h\cdot g)$$

Sum of squares for factor A
represents variation in cell means that can be attributed to factor A
$$SS_{A} = n_{cell} \cdot h \cdot \sum_{j=1}^g (\bar{Y}_{j\bullet} - \bar{Y})^2$$

$$df_A = g-1$$

Sum of squares factor B
represents variation in cell means that is attributed to factor B
$$SS_{B} = n_{cell} \cdot g \cdot \sum_{k=1}^h (\bar{Y}_{\bullet k} - \bar{Y})^2$$

$$df_B = h -1$$


Interaction sum of squares
represents variation in cell means that cannot be attributed to the main effects of factor A and B

If there was no interaction effect, then the following would apply:
$$\bar{Y}_{jk} - \bar{Y}_j - \bar{Y}_k + \bar{Y} = 0$$

$$SS_{A\times B} = n_{cell} \cdot \sum_{k=1}^h \sum_{j=1}^g (\bar{Y}_{jk} - \bar{Y}_{j\bullet} - \bar{Y}_{\bullet k} + \bar{Y})^2$$
$$df_{A \times B} = (g-1) \cdot (h-1)$$

Model Sum of Squares
(as if doing a one-way ANOVA with different contrast)

$$SS_{Model} = SS_A + SS_B + SS_{A\times B}$$


### Non-Orthogonal Factorial ANOVA: Unbalanced Designs ###

control everything that is written here!


- different number of observations per factor-level combination (cell)
- In balanced designs the factors are independent from each other. This is different for non-orthogonal (unbalanced designs). 
- Given unbalanced designs, different procedures can be conducted which lead to not identical results because there is no unique decomposition of the total sum of squares 
- approaches mathematically equivalent to multiple regression with correlated predictors


There are 4 different types of sum of squares.
In balanced designs, all 4 types of sum of squares yield the same result.

### 4 Types of Sum of Squares
#### Type I sum of squares
used by `R`

- order dependent (hierarchical)
- adjusted means
...
- harmonic mean (link to descriptive statistic) of sample sizes
- adjustment of sum of squares as if they were balanced
- unadjusted means


### R application

Make balanced dataset unbalanced
```{r}
skimr::skim(data_indep2)

str(data_indep2)
data_indep2$supp == "OJ"

set.seed(567865) # because taking a sample
data_unbalanced <- 
    data_indep2 |> 
    filter(supp == "OJ") %>%
    slice_sample(n = nrow(.)/2) |> 
    rbind(filter(data_indep2, supp != "OJ"))


skim(data_unbalanced)
```

Standard aov function
```{r}
aov_type1 <- aov(len ~ supp*dose, data = data_unbalanced)
summary(aov_type1)

```

```{r}
aov_type1r <- aov(len ~ dose*supp, data = data_unbalanced) # reversed order
summary(aov_type1r)
```
...
Using base R to get type III sum of squares
```{r collapse=TRUE}
# predictors must be factors
contrasts(data_unbalanced$supp) <- contr.sum(length(levels(data_unbalanced$supp)))
contrasts(data_unbalanced$dose) <- contr.sum(length(levels(data_unbalanced$dose)))

aov_ctr <- aov(len ~ supp*dose, data_unbalanced)
drop1(aov_ctrr, ~., test = "F") # output
aov_ctrr <- aov(len ~ dose*supp, data_unbalanced)
drop1(aov_ctrr, ~., test = "F")
```
output are the same
...
lm approach 
```{r}
aov_lm <- lm(len ~ supp*dose, data_unbalanced)
summary(aov_lm)
```


Using a package
add afex package stuff
```{r}
library(car)
Anova(aov_type1, type = "III")
```
...
-   Sphericity of the dependent factor(s)
-   equality of variance in the independent factor(s)
